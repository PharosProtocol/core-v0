...
contract FillAndClose is TestUtils {
    ...
    Asset USDC_ASSET = Asset({standard:1, addr: 0x6f14C02Fc1F78322cFd7d707aB90f18baD3B54f5, decimals: 18, tokenId: 0, data: ""});
    ...
    bytes constant USDC_ASSET_Encoded = abi.encode(Asset({standard:1,addr: 0x6f14C02Fc1F78322cFd7d707aB90f18baD3B54f5, decimals: 18, tokenId: 0, data: ""}));
    ...
    constructor() {
        ...
        ASSETS.push(Asset({standard:1, addr: 0x6f14C02Fc1F78322cFd7d707aB90f18baD3B54f5, decimals: 18, tokenId: 0, data: ""})); 
    }

    function setUp() public {
        vm.recordLogs();
        vm.createSelectFork(vm.rpcUrl("https://eth-sepolia.g.alchemy.com/v2/2c1VYx8MPBoah8sfNT4b2OOHUwOGfa48"), 4515779); // NOTE ensure this is more recent than deployments.

        // For local deploy of contracts latest local changes.
        bookkeeper = IBookkeeper(0x7d190906465Ea977bBae987ba42b121C0f72515A);
        accountPlugin = IAccount(0xf9bF956503aF95Dc21ef7774314D81d87262E5C9);
        assessorPlugin = IAssessor(0xF699Eae4CF00540b08912D6573d9AeEafD42e02E);
        staticOracle = IOracle(0x0E45D170635E35BA1e2dF77108d1e0366C266207);
        beanDepositOracle = IOracle(address(new BeanDepositOracle()));
        walletFactory = IPosition(0xF3E476A38153Fa55DA91Cc3588e7BFb0d7E8A4Ef);
        ...
        liquidatorPlugin = ILiquidator(0xE1D2dBBC9daE00EB14F6F6d618f8B47E7e154AdC);
    ...
    function test_FillAndClose() public {
        ...
        deal(USDC_ASSET.addr, agreement.position.addr, 5_000 * (10 ** 18), true);
        ...
        console.log("Lender WETH in account",accountPlugin.getBalance(WETH_ASSET_Encoded, abi.encode(lenderAccountParams),""));
        vm.deal(lender, 20e18);
        vm.prank(lender);
        accountPlugin.loadFromUser{value: 12e18}(WETH_ASSET_Encoded, 12e18, abi.encode(lenderAccountParams));
        console.log("Lender WETH in account",accountPlugin.getBalance(WETH_ASSET_Encoded, abi.encode(lenderAccountParams),""));
    ...
    function fundAccount(SoloAccount.Parameters memory accountParams) private {
        ...
        deal(USDC_ASSET.addr, accountParams.owner, 5_000 * (10 ** 18), true);
        ...
        IERC20(0x6f14C02Fc1F78322cFd7d707aB90f18baD3B54f5).approve(address(accountPlugin), 5_000 * (10 ** 18));
    ...
    function fundLiquidator(address liquidator) private {
        ...
        deal(USDC_ASSET.addr, liquidator, 5_000 * (10 ** 18), true);
    ...
    function createOrder(SoloAccount.Parameters memory accountParams) private view returns (Order memory) {
        ...
        collOracles[0] = PluginReference({
            addr: address(staticOracle),
            parameters: abi.encode(StaticOracle.Parameters({number: 1600e18}))
        });
